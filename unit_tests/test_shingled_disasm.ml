...
let test_hits_every_byte test_ctxt =
  ...
  let shingled_disassembly = Shingled_disasm.Conservative.disasm
...
let test_sheers test_ctxt =
  let memory, arch = make_params "\x2d\xdd\xc3\x54\x55" in
  Pervasives.ignore (Shingled_disasm.Conservative.disasm
                       ~accu:[] ~f:List.cons arch memory >>| fun insns ->
                     let (bad, superset_cfg) = Shingled_disasm.cfg_of_shingles insns memory arch in
