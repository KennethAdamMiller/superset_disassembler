...
let check_results sizes expected_results = 
  ...
  print_endline (sizes |> List.to_string ~f:string_of_int);
...
let test_hits_every_byte test_ctxt =
  ...
  let shingled_disassembly = Shingled_disasm.disasm arch memory |> ok_exn in
  let sizes = shingles_to_length_list shingled_disassembly in
