...
fn_check() {
  ...
  if [ ! -f "${o}_stripped_addrs.txt" ]; then
      ./superset_disasm.native --target=${o}_stripped --ground_truth_bin=${f} --save_addrs --tp_threshold=0.99
      mv $(basename ${f})_stripped_addrs.txt ${RQ3_folder}/
  fi
  if [ ! -f "${o}.truth" ]; then
      ./superset_disasm.native --target=${f} --ground_truth_bin=${f} --tp_threshold=0.99 --save_gt > ${o}.truth
  fi
  echo "$(basename ${f}) results: " 
  python ${HOME}/workspace/superset_disasm/scripts/calc_fn.py ${o}.truth ${o}.lserr ${o}_stripped_addrs.txt
  echo ""
}
