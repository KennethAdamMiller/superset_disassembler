...
#    rm -rf "${workdir}"
	...
	#cat ../${src}.txt | while read f ; do   
		...
		cp "${1}" ./
		strip "./$(basename ${1})"
		time ${disasm_dir}/superset_disasm.native --target="./$(basename ${1})" --ground_truth_bin="${1}" --save_addrs --checkpoint=Export --enable_feature="${features}" --rounds=6 --collect_reports >> "${name}";
		rm -f "./$(basename ${1})"
		if [ $? -ne 0 ]; then
